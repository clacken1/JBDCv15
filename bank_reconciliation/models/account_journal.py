...
class AccountJournal(models.Model):
    ...
    bank_statements_source = fields.Selection([
        ...
        ('undefined', 'Undefined')], string='Bank Feeds', default='undefined', help="Defines how the bank statements will be registered")

    def create_bank_statement(self):
        context = self._context.copy()
        """return action to create a bank statements. This button should be called only on journals with type =='bank'"""
        action = self.env.ref(
            'bank_reconciliation.action_bank_statement_wiz').read()[0]
        # action.update({
    ...
    def get_journal_dashboard_datas(self):
        ...
        #                     self.default_debit_account_id.id, self.default_credit_account_id.id] if ac)
        account_ids = tuple(ac for ac in [
                            self.default_account_id.id] if ac)
        if account_ids:
